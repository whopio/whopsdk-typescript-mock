...
export class DisputeGraph {
   ...
   */
  ensureDisputeGraph(record: ResourceRecord, payload: ResourceRecord): ResourceRecord {
    const companyId = (record.company_id ?? payload.company_id) as string | undefined;
    const paymentId = (record.payment_id ?? payload.payment_id) as string | undefined;

    const company = companyId ? this.ensureCompanyExists(companyId) : null;
    const payment = paymentId ? this.ensurePaymentExists(paymentId, companyId) : null;
    ...
    const disputeUpdates: Partial<DisputeRecord> = {
      ...
      status: (record.status ?? payload.status ?? 'needs_response') as DisputeRecord['status'],
      amount: (record.amount ?? payload.amount ?? payment?.total ?? 10.0) as number,
      currency: (record.currency ?? payload.currency ?? payment?.currency ?? 'usd') as string,
      ...
      needs_response_by: (record.needs_response_by ?? payload.needs_response_by ?? this.futureDate(7)) as string,
    ...
    const extraFields: ResourceRecord = {
      ...
      access_activity_log: record.access_activity_log ?? payload.access_activity_log ?? null,
      billing_address: record.billing_address ?? payload.billing_address ?? null,
      cancellation_policy_disclosure: record.cancellation_policy_disclosure ?? payload.cancellation_policy_disclosure ?? null,
      customer_email_address: record.customer_email_address ?? payload.customer_email_address ?? userFromPayment?.email ?? null,
      customer_name: record.customer_name ?? payload.customer_name ?? userFromPayment?.name ?? null,
      notes: record.notes ?? payload.notes ?? null,
      product_description: record.product_description ?? payload.product_description ?? null,
      refund_policy_disclosure: record.refund_policy_disclosure ?? payload.refund_policy_disclosure ?? null,
      refund_refusal_explanation: record.refund_refusal_explanation ?? payload.refund_refusal_explanation ?? null,
      service_date: record.service_date ?? payload.service_date ?? null,
      cancellation_policy_attachment: record.cancellation_policy_attachment ?? null,
      customer_communication_attachment: record.customer_communication_attachment ?? null,
    ...
    const cleanUpdates = this.compactHash({ ...disputeUpdates, ...extraFields });

    this.store.update(ResourceNames.DISPUTE, record.id as string, cleanUpdates);
    return this.store.find(ResourceNames.DISPUTE, record.id as string) ?? record;
  }
   ...
   */
  ensureDisputeAlertGraph(record: ResourceRecord, payload: ResourceRecord): ResourceRecord {
    const companyId = (record.company_id ?? payload.company_id) as string | undefined;
    const paymentId = (record.payment_id ?? payload.payment_id) as string | undefined;
    const disputeId = (record.dispute_id ?? payload.dispute_id) as string | undefined;

    const company = companyId ? this.ensureCompanyExists(companyId) : null;
    const payment = paymentId ? this.ensurePaymentExists(paymentId, companyId) : null;
    const dispute = disputeId ? this.store.find(ResourceNames.DISPUTE, disputeId) : null;

    const now = new Date().toISOString();
    const alertUpdates: Partial<DisputeAlertRecord> & ResourceRecord = {
      created_at: (record.created_at ?? now) as string,
      updated_at: record.updated_at ?? now,
      alert_type: (record.alert_type ?? payload.alert_type ?? 'DISPUTE') as DisputeAlertRecord['alert_type'],
      amount: (record.amount ?? payload.amount ?? payment?.total ?? 10.0) as number,
      currency: (record.currency ?? payload.currency ?? payment?.currency ?? 'usd') as string,
      charge_for_alert: (record.charge_for_alert ?? payload.charge_for_alert ?? true) as boolean,
      transaction_date: (record.transaction_date ?? payload.transaction_date ?? payment?.created_at ?? now) as string,
      payment: payment ? this.compactPayment(payment) : null,
    ...
    this.store.update(ResourceNames.DISPUTE_ALERT, record.id as string, cleanUpdates);
    return this.store.find(ResourceNames.DISPUTE_ALERT, record.id as string) ?? record;
  }
   ...
   */
  ensureResolutionCaseGraph(record: ResourceRecord, payload: ResourceRecord): ResourceRecord {
    const companyId = (record.company_id ?? payload.company_id) as string | undefined;
    ...
    const resolutionUpdates: Partial<ResolutionCaseRecord> & ResourceRecord = {
      ...
      status: (record.status ?? payload.status ?? 'merchant_response_needed') as ResolutionCaseRecord['status'],
      issue: (record.issue ?? payload.issue ?? 'product_not_delivered') as string,
      due_date: (record.due_date ?? payload.due_date ?? this.futureDate(3)) as string,
      customer_appealed: (record.customer_appealed ?? payload.customer_appealed ?? false) as boolean,
      merchant_appealed: (record.merchant_appealed ?? payload.merchant_appealed ?? false) as boolean,
      customer_response_actions: record.customer_response_actions ?? payload.customer_response_actions ?? [],
      merchant_response_actions: record.merchant_response_actions ?? payload.merchant_response_actions ?? ['respond', 'refund'],
      platform_response_actions: record.platform_response_actions ?? payload.platform_response_actions ?? [],
      company: company ? this.compactCompany(company) : null,
      ...
      resolution_events: record.resolution_events ?? payload.resolution_events ?? [],
    };

    const cleanUpdates = this.compactHash(resolutionUpdates);

    this.store.update(ResourceNames.RESOLUTION_CASE, record.id as string, cleanUpdates);
    return this.store.find(ResourceNames.RESOLUTION_CASE, record.id as string) ?? record;
  }
  ...
  private ensurePaymentExists(paymentId: string, companyId?: string): PaymentRecord {
  ...
  private compactResolutionPayment(payment: PaymentRecord): CompactResolutionPayment {
    return {
      id: payment.id,
      currency: payment.currency,
      created_at: payment.created_at,
