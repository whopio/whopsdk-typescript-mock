...
export class PayoutGraph {
  ...
  ensureTransferGraph(
    ...
    const originId = (payload.origin_id ?? record.origin_id) as
      | string
      | undefined;
    const destinationId = (payload.destination_id ?? record.destination_id) as
      | string
      | undefined;
    ...
    const transferUpdates: Record<string, unknown> = {
      ...
      origin_ledger_account_id:
        record.origin_ledger_account_id ?? `ledger_${originId}`,
    ...
    return (
      this.store.find(ResourceNames.TRANSFER, record.id as string) ?? record
    );
  }

  ensureWithdrawalGraph(
    record: ResourceRecord,
    payload: Record<string, unknown>,
  ): ResourceRecord {
    const companyId = (payload.company_id ?? record.company_id) as
      | string
      | undefined;
    const company = companyId ? this.ensureCompanyExists(companyId) : null;
    const payoutMethodId = (payload.payout_method_id ??
      record.payout_method_id) as string | undefined;
    ...
    const withdrawalUpdates: Record<string, unknown> = {
      ...
      ledger_account: {
        id:
          (record.ledger_account as Record<string, unknown>)?.id ??
          `ledger_${companyId}`,
      ...
      payout_token: {
        ...
        payer_name:
          (payoutMethod?.destination as Record<string, unknown>)?.name ??
          company?.title,
      },
    ...
    return (
      this.store.find(ResourceNames.WITHDRAWAL, record.id as string) ?? record
    );
  }

  ensureFeeMarkupGraph(
    record: ResourceRecord,
    payload: Record<string, unknown>,
  ): ResourceRecord {
    ...
    const existing = this.store
      ...
      .find(
        (item) => String(item.fee_type) === feeType && item.id !== record.id,
      );
    ...
    const defaults: Record<string, unknown> = {
      ...
      status:
        record.status === "active"
          ? "paid"
          : (record.status ?? payload.status ?? "paid"),
  ...
  ensureAccountLinkGraph(
    ...
    const useCase = String(
      record.use_case ?? payload.use_case ?? "account_onboarding",
    );
    const expiresAt =
      record.expires_at ??
      payload.expires_at ??
    ...
    this.store.update(
      ResourceNames.ACCOUNT_LINK,
      record.id as string,
      cleanDefaults,
    );
    return (
      this.store.find(ResourceNames.ACCOUNT_LINK, record.id as string) ?? record
    );
  }
  ...
  ensureLedgerAccountGraph(
    ...
    const ledgerAccountUpdates: Record<string, unknown> = {
      ...
      company_id: companyId,
      resource_owner_type:
        record.resource_owner_type ?? payload.resource_owner_type ?? "Company",
      resource_owner_id:
        record.resource_owner_id ?? payload.resource_owner_id ?? companyId,
      ledger_type: record.ledger_type ?? payload.ledger_type ?? "primary",
      withdrawal_status:
        record.withdrawal_status ?? payload.withdrawal_status ?? "active",
      withdrawal_frequency:
        record.withdrawal_frequency ?? payload.withdrawal_frequency ?? "weekly",
      reserve_percentage:
        record.reserve_percentage ?? payload.reserve_percentage ?? 0,
      currency: record.currency ?? payload.currency ?? "usd",
      balance: record.balance ?? payload.balance ?? 0,
      available_balance:
        record.available_balance ?? payload.available_balance ?? 0,
    ...
    this.store.update(
      ResourceNames.LEDGER_ACCOUNT,
      record.id as string,
      cleanUpdates,
    );
    return (
      this.store.find(ResourceNames.LEDGER_ACCOUNT, record.id as string) ??
      record
    );
  }
  ...
  ensurePayoutAccountGraph(
    ...
    const companyId = (record.company_id ?? payload.company_id) as
      | string
      | undefined;
    const ledgerAccountId = (record.ledger_account_id ??
      payload.ledger_account_id) as string | undefined;
    ...
    const payoutAccountUpdates: Record<string, unknown> = {
      ...
      business_name:
        record.business_name ?? payload.business_name ?? company?.title,
      company: company ? this.compactCompany(company) : null,
    };
    ...
    this.store.update(
      ResourceNames.PAYOUT_ACCOUNT,
      record.id as string,
      cleanUpdates,
    );
    return (
      this.store.find(ResourceNames.PAYOUT_ACCOUNT, record.id as string) ??
      record
    );
  }
   ...
   */
  ensurePayoutMethodGraph(
    record: ResourceRecord,
    payload: Record<string, unknown>,
  ): ResourceRecord {
    const payoutAccountId = (record.payout_account_id ??
      payload.payout_account_id) as string | undefined;
    const companyId = (record.company_id ?? payload.company_id) as
      | string
      | undefined;
    ...
    const payoutDestinationId = (record.payout_destination_id ??
      payload.payout_destination_id) as string | undefined;
    ...
    const payoutMethodUpdates: Record<string, unknown> = {
      ...
      destination_currency_code:
        record.destination_currency_code ??
        payload.destination_currency_code ??
        "usd",
      ...
      destination: payoutDestination
        ...
        : (record.destination ?? {
            type:
              record.delivery_type ?? payload.delivery_type ?? "bank_deposit",
            name:
              record.payer_name ??
              payload.payer_name ??
              payoutAccount?.business_name,
          }),
      // Embed payout account info
      payout_account: payoutAccount
        ? this.compactPayoutAccount(payoutAccount)
        : null,
    };
    ...
    this.store.update(
      ResourceNames.PAYOUT_METHOD,
      record.id as string,
      cleanUpdates,
    );
    return (
      this.store.find(ResourceNames.PAYOUT_METHOD, record.id as string) ??
      record
    );
  }
  ...
  private ensureLedgerAccountExists(
    ...
    const ledgerAccount = this.exampleGenerator.generate(
      ResourceNames.LEDGER_ACCOUNT,
      {
        id: ledgerAccountId,
        company_id: companyId,
  ...
  private ensurePayoutAccountExists(
    ...
    const payoutAccount = this.exampleGenerator.generate(
      ResourceNames.PAYOUT_ACCOUNT,
      {
        id: payoutAccountId,
        company_id: companyId,
  ...
  private ensurePayoutDestinationExists(
    payoutDestinationId: string,
  ): ResourceRecord {
    const existing = this.store.find(
      ResourceNames.PAYOUT_DESTINATION,
      payoutDestinationId,
    );
    if (existing) return existing;

    const payoutDestination = this.exampleGenerator.generate(
      ResourceNames.PAYOUT_DESTINATION,
      {
  ...
  private compactPaymentMethod(
    paymentMethod: ResourceRecord,
  ...
  private compactPayoutAccount(
    payoutAccount: ResourceRecord,
  ): Record<string, unknown> {
    return {
      id: payoutAccount.id,
  ...
  private compactPayoutDestination(
    payoutDestination: ResourceRecord,
  ): Record<string, unknown> {
    return {
